...
<span style="text-align: center;"> Question Paper Generator </span> <svg style="color:#0023ff " width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-check2-circle" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  <h3 style="text-align: center;">Enter Question ID to Update it.</h3>
  <div class="row">
    <div class="col-sm-6 d-flex">
      <form action="update_question.php" method="post">
    <div class="input-group mb-3">
  <input type="number" name="que_id" min="1" placeholder="Question ID" class="form-control" aria-label="Text input with segmented dropdown button">
...
<div class="container mt-5 bg-white shadow-lg " style="text-align: center; border-radius: 15px;">
  <div class="row">
      <?php
          ...
          {
            $id = $_POST['que_id'];
            $query_q = "SELECT * FROM question_bank WHERE ID ='$id'";      
            $result = $con->query($query_q);

            if ($result->num_rows > 0)
            {
              $row = $result->fetch_assoc();
              $question_u = $row["question"];
              $solution_u = $row["solution"];

              echo"

                <div class='col-sm-6' style='padding: 15px;'>
                  <h4 style='text-align: center;'>Question ( ID: $id ) <span style='color: red'>*</span></h4>
                  <form class='row' action='update_que_code.php' method='post'>
                  <input type='number' name='u_id' value='$id' style='display:none'>
                  <textarea required name='u_question' style='border-style: solid;border-width: 1px;width: 100%;border-color: grey;height: 200px;padding: 10px;font-size: 14px;'>$question_u</textarea>
                 </div>
                 <div class='col-sm-6' style='padding: 15px;'>
                  <h4 style='text-align: center;'>Solution <span style='color: red'>*</span></h4>
                  <textarea required name='u_solution' style='border-style: solid;border-width: 1px;width: 100%;border-color: grey;height: 200px;font-size: 14px;padding: 10px;'>$solution_u</textarea>
                  <button class='btn btn-success mt-2 mb-3 float-right' type='submit'> Update </button>
                  </form>
